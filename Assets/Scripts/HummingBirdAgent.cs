...
public class HummingBirdAgent : Agent
{
    ...
    [Tooltip("Speed to rotato around up axis")]
    ...
    public override void Initialize()
    {
        ...
        // If not training mode, no max step
    ...
    /// Index 0: move vector x ( +1 = move to right, -1 = move to the left)
    /// Index 1: move vector y (+1 = move up, -1 move down)
    /// Index 2: move vector z (+1 = move forward, -1 = move backwards)
    ...
    public override void OnActionReceived(float[] vectorAction)
    {
        ...
        // Add force inn the direction of the move vector
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        // Observe a normalized vector ponting to the nearest flower (3 observations)
        sensor.AddObservation(toFlower.normalized);

        // Observe a dot product that indicates whether the beak tip is in front of the flower (1 observation)
        // ( +1 means that the beak is directly in front of the flower, -1 means directly behind)
    ...
    /// <summary>
    /// Called when the agents collider stays a trigger collider
    /// </summary>
    /// <param name="other">The trigger collider</param>
        ...
        if(collider.CompareTag("nectar"))
        {
            ...
            if(Vector3.Distance(beakTip.position, closestPointToBeakTip) < BeakTipRadius)
            {
                ...
                if (trainingMode)
                {
                    ...
                    AddReward(0.01f + bonus); // Always give reward if inside of the nectar
                }
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        {
            // Collided with the area boundary, give a negative rewad
            AddReward(-0.05f);
        }
    }
    ...
    {
        // Avois scenario where nearest flower nectar is stolen by opponent and not updated
